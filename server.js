...
// Brand checkout — spends wallet credits into a deal's escrow.
...
app.post('/api/campaigns', auth, async (req, res) => {
  try {
    ...
    // without ever charging the wallet — a brand got a funded-looking deal for free.
    // Paid bookings now go through POST /api/checkout/brief, which debits first.
    if (req.body.selected_creator) {
      return res.status(402).json({
        detail: 'Booking a creator requires payment. Use the checkout to pay for this brief.',
        code: 'PAYMENT_REQUIRED',
        checkout: '/api/checkout/brief',
      });
    }
...
app.get('/api/business/settings/billing', auth, async (req, res) => {
  const commission_rate = await require('./services/pricing').platformFeePercent();
  res.json({ plan: 'Free', commission_rate, next_billing_date: null, monthly_budget: { used: 0, total: 0 } });
});
